...
class PersistenceMessageEndpoint extends EndpointCommon { // the general class forwards publication...
    ...
    async publish(msg_obj) {        // actual pulication... this file becomes available to the general public...
        ...
        if ( status ) {
            this.appliction_meta_publication(msg_obj,this.app_meta_universe)
        }
        return status
    }
    ...
    async unpublish(msg_obj) {        // remove from publication directory... this file becomes unavailable to the general public...
        ...
        if ( status ) {
            this.appliction_meta_remove(msg_obj,this.app_meta_universe)
        }
        return status
    }
    ...
    //   
    meta_publication(msg_obj,app_meta_universe) {
        if ( app_meta_universe ) {
            let data = await this.load_data(msg_obj)
            this.appliction_meta_publication(data)
        }
    }

    //   
    meta_remove(msg_obj,app_meta_universe) {
        if ( app_meta_universe ) {
            let data = await this.load_data(msg_obj)
            this.appliction_meta_remove(data)
        }
    }
    ...
    appliction_meta_remove(msg_obj,app_meta_universe) {
        console.log("the application class should implement appliction_meta_remove")
    }
}
