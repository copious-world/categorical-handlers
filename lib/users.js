...
class UserMessageEndpoint extends EndpointCommon {
    ...
    async init_public_directories() {
        let ok = await this.fos.ensure_directories(this.user_directory)
        if ( !ok ) {
            console.log(`serious error when creating directories ${this.user_directory}`)
            console.log("exiting...")
            process.abort()
        }
        ok = await this.fos.ensure_directories(this.all_users)
        if ( !ok ) {
    ...
    async create_user_assets(u_obj) {
        ...
        //
        let assets = await this.app_asset_generator(u_obj)  // returns an object map to strings or false
        if ( assets ) {
            for ( let asset_key in assets ) {
                ...
                if ( typeof assest_data === "string" ) {
                    await this.fos.write_out_string(file_name,assest_data,{ 'flag' : 'wx' })
                } else {
                    await this.fos.write_out_json(file_name,assest_data,{ 'flag' : 'wx' })
                }
            }
        }
    }
    ...
    async app_message_handler(msg_obj) {
        ...
        switch ( op ) {
            case 'G' : {        // get user information
                let stat = "OK"
                let data = await this.load_data(msg_obj)  // returned as string (not altered)
    ...
    app_asset_generator(u_obj,gen_targets) {
        console.log("the application class should implement app_asset_generator")
        return false
    }
