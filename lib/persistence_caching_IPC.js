...
const EndpointCommon = require("./common_caching_ipc")
//
const DEFAULT_ORIGIN_PUBLIC_PATH = "copious"

// Parent class handles publication 
//
class PersistenceCachingMessageEndpoint extends EndpointCommon { // the general class forwards publication...
    ...
    async publish(msg_obj) {        // actual publication... this file becomes available to the general public...
        let user_path = this.make_path(msg_obj)
        if ( !(user_path) ) return false
        let public_path = this.make_public_path(msg_obj)
        let status = await this.fos.file_copier(user_path,public_path)   // successfully moved to a publication folder (e.g. share with counting service)
        if ( status ) {
    ...
    async unpublish(msg_obj) {        // remove from publication directory... this file becomes unavailable to the general public...
        let public_path = this.make_public_path(msg_obj)
        let status = await this.fos.file_remover(public_path)
        if ( status ) {
    ...
    async meta_publication(msg_obj,app_meta_universe) {
        if ( app_meta_universe ) {
            let d_obj = await this.fos.load_json_data(msg_obj)
    ...
    async delete(msg_obj) {
        ...
        await this.fos.file_remover(user_path)
        await this.fos.file_remover(public_path)
        //
        ...
        switch ( op ) {
            ...
            case 'G' : {        // get user information
                ...
                if ( data === false ) { stat = "ERR"; data = "" }
                    ...
                    data = this.application_data_update(msg_obj,data)  // alter the data object for user consumption... such as removing secrets or control codes
                }
...
module.exports = PersistenceCachingMessageEndpoint
